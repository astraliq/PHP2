...
Vue.component('product', {
    ...
    template: `<div class="product-item">
					...
					<a :href="'#modal' + product.id" name="modal">
					...
					<div :id="'modal' + product.id" class="modalwindow">
						<!-- Заголовок модального окна -->
						<h2 class="modal_title">{{ product.title }}</h2>
						<!-- кнопка закрытия окна определяется как класс close -->
						<a href="#" class="modalwindow_close">X</a>
						<div class="content_modal">
							<img :src="'/img/img_phones/' + product.img" :alt="product.title" :title="product.title" class="modal_img">
						</div>
					</div>
				</div>`
})
