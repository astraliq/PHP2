...
//				this.productsInCart.push(elem);
				this.$parent.putJson(`/index.php`, formData)
                    .then(data => {
                        if(data === 'OK'){
                            find.quantity++;
							this.sumOfProducts();
				...
				this.$parent.postJson(`/index.php`, formData)
				.then(data => {
					if(data === 'OK'){
...
//					this.productsInCart.push(prod);
		...
		resetBasket() {
			...
			this.$parent.deleteJson(`/index.php`, formData)
                    .then(data => {
                        if(data === 'OK'){
                            this.productsInCart = [];
							this.sumOfProducts();
...
//		},
		...
		changeLocationToOrder() {
			if (this.productsInCart.length == 0) {
				return location.href = '/index.php?path=catalog';
			} else {
				...
				<table class="basket-items">
					<tbody class="basket-items_footer">
						<tr class="basket-last-row">
							...
							<button @click="changeLocationToOrder" class="btn-cart-reset btn-cart-order" v-bind:disabled="isButtonDisabled">Оформить заказ</button>
							</td>
